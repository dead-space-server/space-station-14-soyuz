...
public sealed partial class JukeboxMenu : FancyWindow
{
    ...
    private JukeboxSystem _jukeboxSystem; // DS-14
    private readonly List<JukeboxPrototype> _allSongs = new(); // DS-14
    // DS-14 START
    ...
    public JukeboxMenu()
    {
        ...
        SetVolumeSlider(_volume, force: true); // DS-14
    }
    ...
    {
        _audio = audio;
        if (HasActiveLocalVolumeOverride) // DS-14
            TryApplyLocalVolume(_volume); // DS-14
    }
    ...
    {
        _allSongs.Clear(); // DS-14
        _allSongs.AddRange(jukeboxProtos); // DS-14
        _allSongs.Sort(static (left, right) => string.Compare(left.Name, right.Name, StringComparison.OrdinalIgnoreCase)); // DS-14
        RefreshSongList(); // DS-14
    }
