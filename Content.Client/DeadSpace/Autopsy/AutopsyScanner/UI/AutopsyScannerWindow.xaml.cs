...
public sealed partial class AutopsyScannerWindow : DefaultWindow
{
    ...
    public void Populate(AutopsyScannerScannedUserMessage msg)
    {
        ...
        string entityName = msg.EntityName ?? Loc.GetString("autopsy-scanner-window-entity-unknown-text");
        ...
        patientDamageAmount.Text = Loc.GetString(
            ...
            ("amount", msg.TotalDamage)
        );

        var damageSortedGroups = msg.DamagePerGroup
            .OrderBy(d => d.Amount)
            .ToList();

        DrawDiagnosticGroups(damageSortedGroups, msg.DamageSequence.AsEnumerable().Reverse());

        SetHeight = AnalyzerHeight;
        SetWidth = AnalyzerWidth;
    }

    private void DrawDiagnosticGroups(List<DamageGroupEntry> groups, IEnumerable<DamageEventEntry> damageSequence)
    {
        ...
        foreach (var group in groups.AsEnumerable().Reverse())
        {
            ...
            var groupTitleText = $"{Loc.GetString(
                ...
                ("damageGroup", Loc.GetString("autopsy-scanner-window-damage-group-" + group.DamageGroupId)),
                ("amount", group.Amount)
            )}";
            ...
            groupContainer.AddChild(CreateDiagnosticGroupTitle(groupTitleText, group.DamageGroupId, group.Amount.Int()));

            GroupsContainer.AddChild(groupContainer);

            // Show the damage for each type in that group.
            var damageGroup = _prototypes.Index<DamageGroupPrototype>(group.DamageGroupId);

            if (group.DamageGroupId == "Toxin" || group.DamageGroupId == "Genetic" || group.DamageGroupId == "Airloss")
                continue;
