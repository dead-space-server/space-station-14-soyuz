...
namespace Content.Client.Paper.UI
{
    ...
    public sealed partial class PaperWindow : BaseWindow
    {
        ...
        public void Populate(PaperComponent.PaperBoundUserInterfaceState state)
        {
            ...
            if (RuneSheetPanel.Visible)
            {
                // Rune sheets render the rune preview instead of raw paper text.
        ...
        private void UpdateRuneSheetPanel()
        {
            if (_entity == default)
                return;

            if (_entMan.TryGetComponent<RuneBookRuneSheetComponent>(_entity, out var runeSheet) &&
                runeSheet.RuneIndex >= 0 &&
                runeSheet.RuneIndex < RuneBookRuneLibrary.RuneCount)
            {
                RuneSheetPanel.Visible = true;
                RuneSheetPreview.RuneId = runeSheet.RuneIndex;

                if (_entMan.TryGetComponent<MetaDataComponent>(_entity, out var meta))
                    RuneSheetTitle.Text = meta.EntityName;
                else
                    RuneSheetTitle.Text = string.Empty;

                return;
            }

            RuneSheetPanel.Visible = false;
            RuneSheetTitle.Text = string.Empty;
            RuneSheetPreview.RuneId = -1;
        }
