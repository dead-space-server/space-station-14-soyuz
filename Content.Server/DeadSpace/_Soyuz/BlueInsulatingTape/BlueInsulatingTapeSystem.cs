...
using Content.Shared.DeadSpace._Soyuz.BlueInsulatingTape;
...
namespace Content.Server.DeadSpace._Soyuz.BlueInsulatingTape;
...
public sealed class BlueInsulatingTapeSystem : EntitySystem
{
    ...
    private void OnAfterInteract(Entity<WindowRepairTapeComponent> ent, ref AfterInteractEvent args)
    {
        if (args.Handled || !args.CanReach || args.Target is not { } target ||
            !CanRepairWindow(ent, target, out _))
        {
            return;
        }

        var doAfter = new DoAfterArgs(EntityManager, args.User, ent.Comp.Delay,
            new WindowRepairTapeDoAfterEvent(), target, target, ent)
        {
            NeedHand = true,
            BreakOnMove = true,
            BreakOnDamage = true,
    ...
    private void OnDoAfter(Entity<DamageableComponent> target, ref WindowRepairTapeDoAfterEvent args)
    {
        if (args.Handled || args.Cancelled || args.Used is not { } used ||
            !TryComp(used, out WindowRepairTapeComponent? tape) ||
            !CanRepairWindow((used, tape), target, out var stack) || stack is null)
        {
            return;
        }

        var destroyedAt = _destructible.DestroyedAt(target);
        if (destroyedAt <= FixedPoint2.Zero || destroyedAt == FixedPoint2.MaxValue)
            return;

        var healAmount = -(destroyedAt * tape.RepairFraction);
        if (healAmount >= FixedPoint2.Zero || !_stack.TryUse((used, stack), 1))
            return;
    ...
    private bool CanRepairWindow(Entity<WindowRepairTapeComponent> tape, EntityUid target,
        out StackComponent? stack)
    {
        if (!TryComp(tape, out stack) || stack.Count <= 0 ||
            !HasComp<RepairableComponent>(target) || !_tag.HasTag(target, WindowTag))
        {
            return false;
        }
