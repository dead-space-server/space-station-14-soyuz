...
def build_preview(base_sha: str, head_sha: str, output_path: Path, metadata_path: Path | None) -> dict[str, Any]:
    ...
    if not selected_entries:
        ...
        for index, entry in enumerate(selected_entries):
            ...
            # DS14-start: show a single NEW sprite when there was no base asset yet.
            is_new_asset = before_asset is None and after_asset is not None
            if is_new_asset:
                center_x = x + cell_width // 2
                sprite_x = center_x - thumb // 2
                draw.rectangle((sprite_x, top_y, sprite_x + thumb, top_y + thumb), fill=bg, outline=border, width=1)
                image.alpha_composite(fit_sprite(after_asset, thumb), (sprite_x, top_y))
                ...
                new_lines = wrap_text(draw, entry.after_name, small_font, thumb + 80, max_lines=3)
                tag_height = measure_lines_height(draw, ["NEW"], font=tag_font, line_gap=0)
                draw_centered_lines(
                    draw,
                    new_lines,
                    center_x=center_x,
